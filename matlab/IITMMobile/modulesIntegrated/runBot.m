...
% sampleArena=rawArena;
%% PLOT
imshow(sampleArena);
hold on;


% plot(cornerToMove(:,2),cornerToMove(:,1),'rx','LineWidth',2);
% for cornerCount=1:numOfCorners-1
%     line([cornerToMove(cornerCount:cornerCount+1,2)],[cornerToMove(cornerCount:cornerCount+1,1)]);
% end
...
for cornerCount=2:numOfCorners
    ...
    if ( sum((ourRobotFrontCordi - Destination_point_cordinate).^2 ,2) > sum((ourRobotBackCordi - Destination_point_cordinate).^2 ,2) )
        smallCircleIsFront = ~ smallCircleIsFront;
    end

% 
%     if (smallCircleIsFront==0 & sum((ourRobotFrontCordi - Destination_point_cordinate).^2 ,2) > sum((ourRobotBackCordi - Destination_point_cordinate).^2 ,2) )
%         smallCirleIsFront = 0;
%     end
