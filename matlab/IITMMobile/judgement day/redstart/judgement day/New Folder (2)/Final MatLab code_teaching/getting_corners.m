...
for cornerCount=1:maxAllowableCorners
    ...
    if(findBlockFlag==0)% f there is no obstracle we have reached desti nation
        ...
        per_lineBW(4*distance_from_corner+1) = logical(0);
        ...
        if(numOfPaths==0)
            ;%??? i have to think for this case
        elseif(numOfPaths==1)%only one path to go
            indexOfcordi=round(proptiesOfLine.Centroid(1));
            next_point=[perLineArray_X(indexOfcordi) perLineArray_Y(indexOfcordi)];
        else
            clear r;
            clear i;
            for pathCount=1:numOfPaths
                r(pathCount)=abs((proptiesOfLine(pathCount).Centroid-cordiToCheck) *[1 ;i])/proptiesOfLine(pathCount).Area;
                pathCenter(pathCount,:)=round(proptiesOfLine(pathCount).Centroid(1));
            end
