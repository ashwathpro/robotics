...
int main(){
	...
	//cout<<"cam ok...\n";
	IplImage *src;
	src=cvLoadImage("iitbArena1.jpeg",1);
	//src=cvRetrieveFrame(capture);
	
	IplImage *I = cvCreateImage(cvSize(src->width,src->height), 8, 1);
	double tolcirc=0.2;
	...
	while(1)
	{
		...
		for(int i=0;i<height;i++)
		{
			...
			{
				if(srcData[i*src->widthStep + j*channel +0]>=200 && srcData[i*src->widthStep + j*3 +1]<=50
