    ...
    for j=1:length(p)
        ...
        if(i~=j && sqrt( ( (p(i,1)-p(j,1))^2 + (p(i,2)-p(j,2))^2 ) ) <= maxEdgeVal )
            ...
            for k=1:length(centres)
                ...
                side=sqrt( ( (centres(k,1)-p(j,1))^2 + (centres(k,2)-p(j,2))^2 ) ) + sqrt( ( (centres(k,1)-p(i,1))^2 + (centres(k,2)-p(i,2))^2 ) );
                
                inBox=((centres(k,1)> min(p(i,1),p(j,1))) & (centres(k,1) < max(p(i,1),p(j,1))) & (centres(k,2)> min(p(i,2),p(j,2))) & (centres(k,2) < max(p(i,2),p(j,2))) );
