...
% Works in a similar way to the Windows Paint program (except for the ROI image manipulations)
%
% USAGE:
%   MPAINT operates on the current figure.
...
%
%   H = MPAINT(...); returns the handles of the MPAINT figure
%
%   MPAINT works with both RGB and indexed images but the later are converted to RGB using
%   the highly ineficient and memory consuming 'ind2rgb' Matlab base function.
%   
...
%
%   NOTE1: MPAINT is an GUI interface to some functions in the CVLIB_MEX
%   library, so you must have it installed to run this function
%
% EXAMPLES:
%       load clown
%       h = imagesc(X); axis image
%       set(gcf,'colormap',map,'DoubleBuffer','on')
%       mpaint(h);                      % Click at will
%       B = getimage(h);                % Get your work of art
...
function shape_clickedcallback(hObject, eventdata, opt)
    handles = guidata(hObject);     % get handles
    state = uisuspend(handles.hCallingFig);        % Remember initial figure state
...
function pipeta_clickedcallback(hObject, eventdata)
    ...
    if (w == 0)       % A mouse click
        ...
        if (ndims(img) == 2)            % Here we have to permanently change the image type to RGB
            %img = ind2rgb8(img,get(handles.hCallingFig,'ColorMap'));           % USE THIS IF YOU HAVE IPTX
...
function [x,y] = getpixcoords(handles,x,y)
    ...
    x = localAxes2pix(n,handles.XData,x);
    y = localAxes2pix(m,handles.YData,y);
