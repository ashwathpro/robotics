...
function [colorPathIndexed, colorPath, weightColor]= findColorPath(color_index,mag_index,startNode,endNode,miniArena)
icnt=0;
...
%     disp([h k]);
	...
	if((miniArena(h,k)==color_index || miniArena(h,k)==mag_index) && (colorPath(h,k)~=255))
        ...
        if(miniArena(h,k)==mag_index)
            weightColor=weightColor+10;
        else weightColor=weightColor+5;
        end
    end
