...
list=['black  ';'red    ';'white  ';'magenta';'blue   ';'green  ';'yellow ';'orange '];
listc=char(list);
...
if numel(Clusters)==1
	...
	for ii=1:Clusters
		figure(tmpfig);
		title(sprintf('Select sample pixel(s) in cluster %d of %d(%s):',ii,Clusters,deblank(listc(ii,:))));
% 		tmp = impixel;
%        tmp=[0 0 0 ];
       tmp=meanPoints(imgin);    
%        tmp= [ tmp; meanPoints(imgin) ]
%        tmp= [ tmp; meanPoints(myim) ]
...
% 		assignin('base',['clusteredMap' num2str(n)],colors);
        assignin('base','arenaIndexed',pixel_labels);
 		assignin('base','arenaMap',colors);
	end
