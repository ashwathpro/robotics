function varargout = hue_sat_light_gui(varargin)

%============================== Example ====================================
%[close_status image_output slider_pos1 slider_pos2 slider_pos3 ok_status cancel_status preview_status]=hue_sat_light_gui(img2,-0.3423,-0.11,0.22,1);
%===========================================================================
%========================= INTIALISATION ===================================
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @hue_sat_light_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @hue_sat_light_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function hue_sat_light_gui_OpeningFcn(hObject, eventdata, handles, varargin)
...
uiresume
set(handles.hue_sat_light_figure,'Pointer','watch');
set(handles.edit1,'String',int2str(get(hObject,'Value')*100));
pause(0.001); %=>for the change to show up in gui.
hue_value=get(handles.slider1,'Value');
saturation_value=get(handles.slider2,'Value');
lightness_value=get(handles.slider3,'Value');
output_image=hsl( cell2mat(handles.output(1)), [hue_value saturation_value lightness_value] );
...
uiresume
set(handles.hue_sat_light_figure,'Pointer','watch');
[FileName,PathName]=uigetfile({'*.mhs','MATLAB Hue/Sat. Settings (*.MHS)'}, 'Load');
if(PathName~=0)
...
pause(0.001); %=>for the change to show up in gui.
output_image=hsl( cell2mat(handles.output(1)) , settings );
set(handles.ok,'UserData',output_image);
end
%==========================================================================
%======================== SAVE SETTINGS ===================================
...
[FileName,PathName]=uiputfile({'*.mhs','MATLAB Hue/Sat. Settings (*.MHS)'}, 'Save');
if(PathName~=0)
file_link=[PathName FileName '.mhs'];
...
%==========================================================================
%===================== GUI OUTPUT FUNCTION ================================
function varargout = hue_sat_light_gui_OutputFcn(hObject, eventdata, handles)
uiwait
%--------------------------------------------------------------------------
...
delete(handles.hue_sat_light_figure);
end
%==========================================================================
%##########################################################################
%---------------------------Extra Used Function(s)-------------------------
%**************************** HSL *****************************************
function output_image=hsl(input_image,hsl_array)
[m n r]=size(input_image);
img=rgb2hsv(input_image);
img(:,:,1)=img(:,:,1)+hsl_array(1)*0.5;
img(:,:,2)=img(:,:,2)+hsl_array(2)*1;
img(:,:,3)=img(:,:,3)+hsl_array(3)*1;
img=hsv2rgb(img);
