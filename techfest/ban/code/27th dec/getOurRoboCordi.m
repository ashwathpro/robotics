...
while(1)
    ...
    if(full_image==0)% if search in small bax
        upper_box_point = max(min(g_OurRobotBlackCordi,g_OurRobotWhiteCordi)- g_robotBoxDimentions,int16([1 ,1]));
        lower_box_point = min(max(g_OurRobotBlackCordi,g_OurRobotWhiteCordi)+ g_robotBoxDimentions,...
                              int16([size_input_colored_image(2),size_input_colored_image(1)]));
