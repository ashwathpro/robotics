...
if(full_image==0)
    upper_box_point = min(g_OpponentRobotBlackCordi,g_OpponentRobotWhiteCordi)-g_robotLargerBoxDimentions;
    lower_box_point= max(g_OpponentRobotBlackCordi,g_OpponentRobotWhiteCordi)+ g_robotLargerBoxDimentions;
    ...
    upper_box_point = int16([1 1]);
    lower_box_point = int16([size_input_colored_image(2) size_input_colored_image(1)]);
...
upper_opponentRoboPoint = int16(bbOpponentRobot(1:2))+ upper_box_point - int16([1 1]);
lower_opponentRoboPoint = int16(bbOpponentRobot(1:2) +bbOpponentRobot (3:4))- int16([1 1])+ upper_box_point - int16([1 1]);
...
if(NOpponentRobotW==0)
    fprintf('***** ERROR: White Color of Opponent robot  color not detected *****')
    return
end
...
if(NOpponentRobotW~=1)
    ...
    g_OpponentRobotWhiteCordi =  int16(regionPropertiesOpponentRobotW(indexOpponentRobotW).Centroid) + upper_opponentRoboPoint-1;
else
    g_OpponentRobotWhiteCordi =  int16(regionPropertiesOpponentRobotW.Centroid) + upper_opponentRoboPoint-1;
end
...
if(NOpponentRobotB~=1)
    ...
    g_OpponentRobotBlackCordi =  int16(regionPropertiesOpponentRobotB(indexOpponentRobotB).Centroid) + upper_opponentRoboPoint-1;
else
    g_OpponentRobotBlackCordi =  int16(regionPropertiesOpponentRobotB.Centroid )+ upper_opponentRoboPoint-1;
end

BWOpponentRobot =logical(zeros(size_input_colored_image(1),size_input_colored_image(2)));
BWOpponentRobot(upper_opponentRoboPoint(2):lower_opponentRoboPoint(2),upper_opponentRoboPoint(1):lower_opponentRoboPoint(1)) = opponentRoboImage;
